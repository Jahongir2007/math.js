...
var get_met = $("sign").attr("get:val");
var give_val = $("sign").text(Math.sign(get_met));
var get_met = $("log2").attr("get:val");
var give_val = $("log2").text(Math.log2(get_met));
var get_met = $("fround").attr("get:val");
var give_val = $("fround").text(Math.fround(get_met));
...
var get_met = $("atan").attr("get:val");
var give_val = $("atan").text(Math.atan(get_met));
var get_met = $("exp").attr("get:val");
var give_val = $("exp").text(Math.exp(get_met));
var get_met = $("expm1").attr("get:val");
var give_val = $("expm1").text(Math.expm1(get_met));
var get_met = $("floor").attr("get:val");
var give_val = $("floor").text(Math.floor(get_met));
var get_met = $("atanh").attr("get:val");
var give_val = $("atanh").text(Math.atanh(get_met))
...
var get_met = $("ceil").attr("get:val");
var give_val = $("ceil").text(Math.ceil(get_met));
...
var get_aside = $("per").attr("p:a");
var get_bside = $("per").attr("p:b");
var val_giveper = $("per").text(Number(get_aside) + Number(get_bside) + Number(get_aside) + Number(get_bside)); 
var gets_aside = $("sq").attr("s:a");
var gets_bside = $("sq").attr("s:b");
var val_gives = $("sq").text(Number(gets_aside) * Number(gets_bside)); 
var getv_h = $("v").attr("v:h");
var getv_s = $("v").attr("v:s");
var val_v = $("v").text(Number(getv_h) * Number(getv_s));
var getc_r = $("c").attr("c:r");
var val_c = $("c").text(2 * Math.PI * Number(getc_r)); 
var gets_r = $("sph").attr("sph:r");
var val_c = $("sph").text(Math.PI * Number(gets_r) * Number(gets_r)); 
var get_inqvalf = $("inq").attr("inq:val1");
var get_inqvals = $("inq").attr("inq:val2");
if(get_inqvalf < get_inqvals){
    var g_val = get_inqvals  + " number is large then " + get_inqvalf;
}else if(get_inqvalf > get_inqvals){
    var g_val = get_inqvalf  + " number is large then " + get_inqvals;
}else if(get_inqvalf == get_inqvals){
    var g_val = "the numbers are equal";
}else{
    var g_val ="Error!"
};
var get_cyvs = $("cyv").attr("cyv:s");
var get_cyvh = $("cyv").attr("cyv:h");
var get_cyv = $("cyv").text(Number(get_cyvs) * Number(get_cyvh));
var w_val = $("inq").text(g_val);
$("pi").text(Math.PI);
$("e").text(Math.E);
$("ln2").text(Math.LN2);
$("ln10").text(Math.LN10);
